...
public class Box2DExample extends GdxExample implements InputProcessor {
    ...
    public void create() {
        ...
        this.camera = new OrthographicCamera(48, 32);
    ...
    private void createPhysicsWorld() {
...

        {
            ...
            // around the origin
            PolygonShape groundPoly1 = new PolygonShape();
            groundPoly1.setAsBox(50, 1, new Vector2(0, -16), 0);
            PolygonShape groundPoly2 = new PolygonShape();
            groundPoly2.setAsBox(50, 1, new Vector2(0, 16), 0);
            PolygonShape groundPoly3 = new PolygonShape();
            groundPoly3.setAsBox(1, 34, new Vector2(-24, 0), 0);
            PolygonShape groundPoly4 = new PolygonShape();
            groundPoly4.setAsBox(1, 34, new Vector2(24, 0), 0);
            ...
            fixtureDef.shape = groundPoly1;
            this.groundBody.createFixture(fixtureDef);

            fixtureDef.shape = groundPoly2;
            this.groundBody.createFixture(fixtureDef);

            fixtureDef.shape = groundPoly3;
            this.groundBody.createFixture(fixtureDef);

            fixtureDef.shape = groundPoly4;
            this.groundBody.createFixture(fixtureDef);
...
//  System.out.println("post solve, normal impulses: " + ni[0] + ", " + ni[1] + ", tangent impulses: " + ti[0] + ", " + ti[1]);
    ...
    public void render() {
        ...
        this.renderBox(this.groundBody, 50, 1);
        ...
        for (int i = 0; i < this.boxes.size(); i++) {
            ...
            this.batch.draw(this.textureRegion, position.x - 1, position.y - 1, // the bottom left corner of the box, unrotated
        ...
        this.batch.getProjectionMatrix().setToOrtho2D(0, 0, Gdx.graphics.getWidth(), Gdx.graphics.getHeight());
        this.batch.begin();
        this.font.draw(this.batch, "fps: " + Gdx.graphics.getFramesPerSecond() + " update time: " + updateTime, 0, 20);
        this.batch.end();
    }
    ...
    private void renderBox(Body body, float halfWidth, float halfHeight) {
        ...
        this.transform.setToTranslation(pos.x, pos.y, 0);
        this.transform.rotate(0, 0, 1, (float) Math.toDegrees(angle));

        // render the box
        this.renderer.begin(ShapeType.Line);
        this.renderer.setTransformMatrix(transform);
        this.renderer.setColor(1, 1, 1, 1);
        this.renderer.rect(-halfWidth, -halfHeight, halfWidth * 2, halfHeight * 2);
        this.renderer.end();
    }
    ...
    public boolean touchDown(int x, int y, int pointer, int newParam) {
        ...
        this.hitBody = null;
        this.world.QueryAABB(this.callback, this.testPoint.x - 0.1f, this.testPoint.y - 0.1f,
                this.testPoint.x + 0.1f, this.testPoint.y + 0.1f);
        ...
        if (this.hitBody != null) {
            ...
            this.mouseJoint = (MouseJoint) this.world.createJoint(def);
            this.hitBody.setAwake(true);
        } else {
            //for (Body box : this.boxes)
            //    this.world.destroyBody(box);
            //this.boxes.clear();
            //createBoxes();
            this.createBox(this.testPoint.x, this.testPoint.y);
        }
    ...
    public boolean touchDragged(int x, int y, int pointer) {
        ...
        if (this.mouseJoint != null) {
            this.camera.unproject(this.testPoint.set(x, y, 0));
